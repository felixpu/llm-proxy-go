...
# 核心链路热路径基准（输出到 .benchmarks/）
make bench-hotpath

# 严格 A/B 对比报告（benchstat）
make bench-ab
```

最新报告示例：

- `docs/benchmarks/hotpath_ab_20260414_134522.md`
...
路由缓存说明：
- 当前 Go 实现使用精确匹配缓存，不包含基于嵌入向量的语义相似度检索
- L1 命中后直接返回，L2 用于跨进程/重启后的持久化复用
